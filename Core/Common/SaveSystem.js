...
export class SaveSystem {
  ...
  showSaveLoadScreen(isLoadMode = true) {
    const screen = this.vnEngine.UI.GetUIElement('save-load-screen');
    const grid = this.vnEngine.UI.GetUIElement('save-load-grid');
    // @ts-ignore
    grid.innerHTML = '';
    for (let i = 1; i <= 8; i++) {
      const slotName = `slot${i}`;
      const div = html`<div class="save-slot">
        <label>Slot ${i}</label>
      </div>`
      ...
      if (isLoadMode) {
        if (!savedState) {
          ...
          const bg = html`${savedState.lastBackground}`          
          div.append(bg[bg.length - 1])
